...
function sendEmail($to, $subject, $messageBody) {
    ...
    try {
        ...
        $mail->Host       = 'mail.omabracredit.co.ke';                 // Set the SMTP server to send through
        $mail->SMTPAuth   = true;                                 // Enable SMTP authentication
        $mail->Username   = 'notifications@omabracredit.co.ke';                // SMTP username
        $mail->Password   = 'b2Ps]s-}?CB.';                       // SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;       // Enable ssl encryption
        $mail->Port       = 465;                                  // TCP port to connect to

        // Recipients
        $mail->setFrom('notifications@omabracredit.co.ke', 'Omabra Credit');
...
function getClientIdByPhoneNumber($phone_number) {
    global $conn;
    // Prepare SQL query to get client_id based on phone_number
    $sql = "SELECT client_id FROM clients WHERE phone_number = ?";
    
    // Prepare the statement
    if ($stmt = $conn->prepare($sql)) {
        // Bind the phone_number to the query
        ...
        if ($result->num_rows > 0) {
            ...
            return "N/A"; // No client found for the provided phone_number
        }
    } else {
        // Return "N/A" if SQL preparation fails
        error_log("Error preparing statement: " . $conn->error);
        return "N/A";
    }
}
...
function getNextPaymentAmount($loan_id) {
    ...
    // Get the total due amount and default to 0 if it's null
    $total_due = $row['total_due'] ?? 0;

    // Fetch the loan balance using the calculateLoanBalance function and default to 0 if it's null
    $loanBalance = calculateLoanBalance($loan_id) ?? 0;

    // If total_due is greater than loanBalance, return loanBalance; otherwise, return total_due
    return ($total_due > $loanBalance) ? $loanBalance : $total_due;
}
...
function calculateClearTodayAmount($loan_id) {
    ...
    // Fetch the total installment amount from expected payments for the given loan
    $sql1 = "SELECT SUM(installment_amount) AS total_installment_amount FROM expected_payments WHERE loan_id = ?";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param("s", $loan_id);
    $stmt1->execute();
    $result1 = $stmt1->get_result();
    $row1 = $result1->fetch_assoc();
    $totalInstallmentAmount = $row1['total_installment_amount'] ?? 0;

    // Fetch the total amount paid from payments for the given loan
    $sql2 = "SELECT SUM(amount) AS total_paid_amount FROM payments WHERE loan_id = ?";
    $stmt2 = $conn->prepare($sql2);
    $stmt2->bind_param("s", $loan_id);
    $stmt2->execute();
    $result2 = $stmt2->get_result();
    $row2 = $result2->fetch_assoc();
    $totalPaidAmount = $row2['total_paid_amount'] ?? 0;

    // Calculate the difference (amount to clear today)
    $clearTodayAmount = $totalInstallmentAmount - $totalPaidAmount;

    // Return the amount to clear today
    return max($clearTodayAmount, 0); // Ensure it doesn't return a negative value
}
