...
// Prepare SQL queries
// Query to get loans in 'Loan Loss Provision Unrecognised' stage
$sqlLoans = "
    SELECT ls.loan_id
    FROM loan_stage ls
    WHERE ls.stage = 'Loan Loss Provision Unrecognised'
";
...
if ($resultLoans->num_rows > 0) {
    ...
    while ($row = $resultLoans->fetch_assoc()) {
        ...
        if ($amount > 0) {
            ...
            $sqlInsertOc24entries = "
                ...
                VALUES (4, '$loanNumber', '$today', $amount, $amount, 'Provision for potential loss on loan $loanId')
            ";
            ...
            if ($conn->query($sqlInsertOc24entries) === TRUE) {
                ...
                $sqlInsertOc24entryitems1 = "
                    ...
                    VALUES ($entryId, 29, $amount, 'D')
                ";

                $sqlInsertOc24entryitems2 = "
                    INSERT INTO oc24entryitems (entry_id, ledger_id, amount, dc)
                    VALUES ($entryId, 30, $amount, 'C')
                ";
                ...
                $sqlUpdateLoanStage = "
                    UPDATE loan_stage
                    SET stage = 'Loan Loss Provision Recognised'
                    WHERE loan_id = '$loanId'
                ";
    ...
    logMessage("No loans in 'Loan Loss Provision Unrecognised' stage.");
}
