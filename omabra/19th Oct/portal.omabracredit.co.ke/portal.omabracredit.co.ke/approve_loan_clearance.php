...
if (isset($_POST['loan_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE loan_info SET loan_status = 'Cleared' WHERE loan_id = ?");
    $stmt->bind_param("s", $loan_id);

    if ($stmt->execute()) {
        // If successful, return a success message or redirect
        $loanStatusUpdateSuccess = 'Loan status updated to Cleared.';
        //echo json_encode(['status' => 'success', 'message' => 'Loan status updated to Cleared.']);
        header("Location: loan-clearance.php");
    } else {
