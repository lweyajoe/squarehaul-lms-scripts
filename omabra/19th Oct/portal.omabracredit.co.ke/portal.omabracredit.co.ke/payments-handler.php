...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            $message .= "Option 2: Make payments to $bankName, No. $accountNumber, Account: $loanIdPost, and this should read $payeeName as the payee.\n";
            $message .= "Option 3: Make payments to Equity Bank at: Account Name: Omabra Limited Account Number: 0260272825664\n\n";
			$message .= "Best regards,\nLoan Administrator";
			
			sendEmail($to, $subject, $message);

            $phoneNumber = getPhoneNumber($loanIdPost);

            $atPhoneNumber = formatPhoneNumber($phoneNumber); // Client's phone number
            $smsMessage = "Dear $clientName, your payment for loan $loanIdPost has been received. Your pending balance is Kshs $balance .Thank you for choosing us!";
...
<?php
// Sort the $loanSummaries array by 'loan_id' in descending order
usort($loanSummaries, function ($a, $b) {
    return strcmp($b['loan_id'], $a['loan_id']);
});
?>
...
<li>
    <form id="paymentForm-<?php echo htmlspecialchars($summary['loan_id']); ?>">
        <div class="form-group">
            <label>Paying a different amount?</label>
            <input 
                type="text" 
                class="form-control" 
                name="amount" 
                id="customAmount-<?php echo htmlspecialchars($summary['loan_id']); ?>" 
                required 
            />
        </div>
        <button 
            type="button" 
            class="btn btn-success btn-sm btn-block"
            onclick="triggerSTKPushOwnAmount(
                '<?php echo $summary['loan_id']; ?>', 
                '<?php echo htmlspecialchars($phoneNumber ?? ''); ?>',
                document.getElementById('customAmount-<?php echo htmlspecialchars($summary['loan_id']); ?>').value
            )"
        >
            Make Payment via MPESA
        </button>
...
function triggerSTKPush(loan_id, phone_number) {
    const promptId = 'mpesaPinPrompt-' + loan_id;
...
function triggerSTKPushOwnAmount(loan_id, phone_number, amount) {
    const promptId = 'mpesaPinPrompt-' + loan_id;
    document.getElementById(promptId).innerHTML = '<p>Please wait... Sending STK Push request.</p>';

    // Send the STK Push request via AJAX
    fetch('api/stk_push_own_amount.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function startPolling(loan_id, promptId) {
    // Poll every 5 seconds to check payment status
    pollingInterval = setInterval(() => {
        fetch(`api/check_payment_status.php?loan_id=${loan_id}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'paid') {
                clearPolling();
                document.getElementById(promptId).innerHTML = '<p>Payment received successfully. Thank you!</p>';
            } else if (data.status === 'pending') {
                console.log('Payment still pending...');
            } else {
                clearPolling();
                document.getElementById(promptId).innerHTML = '<p>Error: Payment failed or timed out.</p>';
            }
        })
        .catch(error => {
            clearPolling();
            document.getElementById(promptId).innerHTML = '<p>Error checking payment status.</p>';
        });
...
function startTimeout(promptId) {
    ...
    timeout = setTimeout(() => {
        ...
        document.getElementById(promptId).innerHTML = '<p>Payment timed out. If not paid, please try again.</p>';
