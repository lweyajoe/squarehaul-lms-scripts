...
include_once "approve_loan_clearance.php";
...
while ($loan_info = $active_loans_result->fetch_assoc()) {
    ...
    // Check if the loan is eligible for clearance (total payments >= total amount due)
    $total_payments = floatval($loan_status_info['Total Payments']);
    $total_amount_due = floatval($loan_status_info['Total EIs']); // If no commas are involved, floatval() is simpler

    if ($total_payments >= $total_amount_due) {
        ...
        <div class="main-container">
			...
			<div class="min-height-200px">
                        ...
                        <div class="col-md-12 col-sm-12">
                            ...
                            <nav aria-label="breadcrumb" role="navigation">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Loan Profiles Of Loans Ready For Clearance</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                ...
                <?php if ($loans_found): ?>
                                ...
                                <div class="pd-20 card-box height-100-p">
                                    ...
                                    <!-- Approve Loan Clearance Button -->
                                     <div class="text-center mt-4">
                                        <form method="POST" action="approve_loan_clearance.php" class="approve-loan-form">
                                            <input type="hidden" name="loan_id" value="<?php echo htmlspecialchars($profile['loan_info']['loan_id']); ?>">
                                            <div class="form-group mb-0">
                                                <button type="submit" class="btn btn-success">Approve Loan Clearance</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                    ...
                    <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-center">No loans available for clearance.</p>
                        <?php endif; ?>
                    </div>
